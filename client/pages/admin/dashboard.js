...
export default function AdminDashboard() {
    ...
    const fetchData = async () => {
        try {
            ...
            const [feedRes, stuRes, wasteRes, compRes, eventRes, predictRes] = await Promise.all([
                ...
                axios.get(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5002'}/api/students`, config),
                axios.get(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5002'}/api/waste`, config),
                axios.get(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5002'}/api/complaints`, config),
                axios.get(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5002'}/api/events`, config),
                axios.get(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5002'}/api/analytics/predict`, config)
            ]);
    ...
    const handleLogWaste = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const config = { headers: { 'x-auth-token': token } };
            const res = await axios.post(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5002'}/api/waste`, {
                date: wasteDate,
                amount: wasteAmount
            }, config);
    ...
    const handleSendNotif = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.post(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5002'}/api/notifications`, { message: notifMsg, type: notifType }, { headers: { 'x-auth-token': token } });
    ...
    const handleCreateEvent = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.post(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5002'}/api/events`, { title: eventTitle, description: eventDesc }, { headers: { 'x-auth-token': token } });
